  ...
  return (
    <article className="mx-auto max-w-3xl px-6 py-10 md:py-14 animate-fade-in">
      <header className="mb-8 pb-6 border-b border-border">
        ...
        <h1 className="font-sans text-3xl md:text-4xl font-bold tracking-tight leading-tight">{lesson.title}</h1>
        {lesson.subtitle && (
          <p className="mt-3 font-serif text-lg italic text-muted-foreground">{lesson.subtitle}</p>
        )}
      </header>
      ...
      <footer className="mt-16 pt-8 border-t border-border">
        <div className="flex items-center justify-between mb-6">
          ...
          >
            ...
            {lessonState?.status === 'completed' ? 'Marked complete ✓' : 'Mark lesson complete'}
          </Button>
        </div>
