...
export function Quiz({ question, choices }: QuizProps) {
  ...
  return (
    <div className="my-6 rounded-lg border border-border bg-card p-5">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-xs font-mono uppercase tracking-wider text-primary/80">Self-check</span>
      </div>
      <p className="font-serif text-[1.05rem] leading-relaxed text-foreground mb-4">{question}</p>
        ...
        {choices.map((c, i) => {
          ...
          return (
            <button
              ...
              className={cn(
                'w-full text-left rounded-md border px-4 py-3 transition-all hover:bg-muted/50',
            ...
            >
              <div className="flex items-start gap-3">
                <div className={cn(
                  ...
                  isSelected && !isCorrect && 'border-destructive bg-destructive',
                  !isSelected && 'border-muted-foreground/30'
                )}>
                  {showResult && isCorrect && <Check className="h-3 w-3 text-white" />}
                  {showResult && !isCorrect && <X className="h-3 w-3 text-white" />}
                </div>
