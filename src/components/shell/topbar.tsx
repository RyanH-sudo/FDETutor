...
export function TopBar() {
  ...
  const toggleTheme = () => {
    ...
    localStorage.setItem('fdetutor-theme', next ? 'dark' : 'light');
  };
  ...
  return (
    <header className="h-14 border-b border-border flex items-center justify-between px-4 md:px-6 bg-background/80 backdrop-blur-sm sticky top-0 z-30">
      <div className="flex items-center gap-3">
        <Link href="/" className="md:hidden font-sans font-semibold tracking-tight">FDETutor</Link>
      </div>
      <div className="flex items-center gap-1">
        <Button asChild variant="ghost" size="icon" aria-label="Glossary">
          <Link href="/glossary/"><Search className="h-4 w-4" /></Link>
        </Button>
